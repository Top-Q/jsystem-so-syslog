...
public class Syslog extends CliApplication {
	...
	private String fetch() throws Exception {
		...
		sendAnyCommandSilent("cp -f " + this.getLogFile() + " "
				+ tmpFileDirectory + tmpFileName);
		sendAnyCommandSilent("ls -l " + tmpFileDirectory + tmpFileName);

		// permision that every body can read write and delete this file
	...
	private BigInteger getFileSize(String fname) throws Exception {
		...
		String numberAsString = (tc.getCounter().replaceAll("\\p{Space}+", " "))
				.split(" ")[4];
	...
	public boolean isGrow(String fname, int seconds) throws Exception {
		...
		if (getFileSize(fname).compareTo(size1) == 1)
			return true;
		return false;
	}
	...
	private void fixSyslogConf1() throws Exception {
		...
		// File aFile = new File(getSyslogConfFile());
		...
		// }
		//
		// if (contents.indexOf("Local0.*")==-1)
		// {
		// sendAnyCommand("echo "+this.getSyslogLine()+" >>
		// "+this.getSyslogConfFile());
		// }

		// sendAnyCommand("cat "+this.getSyslogConfFile());
		String syslogAddLine = "local0.* +" + this.getLogFile();// /usr/Cooter/tmp/setup_H_syslog.log";
		String cmd = "cat " + this.getSyslogConfFile1() + " | egrep \""
				+ syslogAddLine + "\"";
		sendAnyCommand(cmd);
		String tmpTestAgainst = this.getTestAgainstObject().toString();
		String tmpTA2 = tmpTestAgainst.substring(tmpTestAgainst.indexOf(cmd)
				+ cmd.length(), tmpTestAgainst.length());
		syslogAddLine = syslogAddLine.replace("+", "     ");
		if (!(tmpTA2.replaceAll(" +", " ").contains(syslogAddLine.replaceAll(
				" +", " "))))
			sendAnyCommand("echo \"" + syslogAddLine + "\" >> "
					+ syslogConfFile1);
	}
	...
	private void fixSyslogConf2() throws Exception {
		...
		String cmd = "cat " + this.getSyslogConfFile2() + " | egrep "
				+ searchLine;
		sendAnyCommand(cmd);

		/** think to put it in function - also at first config */
		String tmpTestAgainst = this.getTestAgainstObject().toString();
		String tmpTA2 = tmpTestAgainst.substring(tmpTestAgainst.indexOf(cmd)
				+ cmd.length(), tmpTestAgainst.length());
		tmpTA2 = tmpTA2.split("\\r\\n")[1].replaceAll("\\r", "");
		...
		if (!tmpTA2.equals(shouldBeLine)) {
			...
			sendAnyCommand("sed 's/" + badLine + "/" + shouldBeLine + "/g' "
					+ this.getSyslogConfFile2() + " > " + tmpFileName);
			sendAnyCommand("mv -f " + tmpFileName + " "
					+ this.getSyslogConfFile2());
		}
	}
	...
	public void startSyslog() throws Exception {
		...
		// startSyslogProcess();
		if (!isGrow())
			report.report("syslog file is not growing", false);
		clearLog();
	}
